...
void shiftChannels(unsigned char* pixelData, int totalBytes, int shiftR, int shiftL) {
    ...
    // Puedes aplicar diferentes desplazamientos para cada canal si lo deseas.
    // Aquí se ejemplifica para todos los canales:
    for (int i = 0; i < totalBytes; ++i) {
        // Desplazar a la izquierda o derecha.
        // Por ejemplo, si shiftR > 0, se desplaza a la derecha; si shiftL > 0, a la izquierda.
        // Combina las operaciones según el efecto deseado.
        // Este ejemplo aplica un desplazamiento a la derecha de 1 bit:
        pixelData[i] >>= 1;
        // O, para desplazar hacia la izquierda (con precaución con el desbordamiento):
        // pixelData[i] <<= 1;
    }
}
